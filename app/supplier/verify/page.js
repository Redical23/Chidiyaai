    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (files.gstCertificate) {
                uploadPromises.push(uploadFile(files.gstCertificate).then(url => { if (url) uploadedUrls.gstCertificate = url; }));
            }
            if (files.panCard) {
                uploadPromises.push(uploadFile(files.panCard).then(url => { if (url) uploadedUrls.panCard = url; }));
            }
            if (files.iecCertificate) {
                uploadPromises.push(uploadFile(files.iecCertificate).then(url => { if (url) uploadedUrls.iecCertificate = url; }));
            }
            if (files.industryLicense) {
                uploadPromises.push(uploadFile(files.industryLicense).then(url => { if (url) uploadedUrls.industryLicense = url; }));
            }

            await Promise.all(uploadPromises);

            // 2. Submit Verification Data with Image URLs
            const res = await fetch("/api/supplier/verify", {
                ...
                body: JSON.stringify({
                    ...
                    otherLicenses: formData.otherLicenses,
                    // Pass the uploaded URLs
